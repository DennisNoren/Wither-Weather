...
#### NOTE: Bug fixed on 3/29/20. This affected the daily record min-max values only. The software was previously grabbing the wrong station in many cases, the one whose station ID is alphabetically before the others close to that city, rather than the one that is closest in distance.

##### NOTE: TBS bug fix. There are many weather stations whose daily record min-max have incorrect values, sometimes wildly wrong. These are errors in the source data, but the data needs to be cleaned. The plan is to do this as furhter pre-processing the file that contains these data. This pre-processing will, for each weather station, compute a moving average for the record daily maximum and a moving standard deviation for the record daily maximum. Then it will determine if the reported value is higher than the moving average by more than k std deviations, k to be determined by experimentation. The original reported value will be replaced by the moving average in those cases. The same process will be used for reported values that are far less than the moving average, and further compare it against the "normals" daily maximum. The value of k may be different for this.

##### The same process will be done with the daily minimum values. Again, the k values may be different. The experimentation will include looking at nearby weather stations to see if they have a similar extreme value. If they do, it may be an indicator that this was a legitimate extreme temperature event, and should not be changed. The goal is to have one process to auto-correct these values rather than go look at them individually; after all, there are nearly 4600 weather stations.
...
##### - Displays the selected cities on a map. Allows user to view locations/names of other cities in dataset and locations of all weather stations in dataset.
...
### Plans (Potentially) for Bug Fixes and New Capabilities

#### In order of priority:  

#### Fix alignment problem between shaded relief background and state boundaries. This is probably related to map projection.  
#### Change to a different map projection that is equal area.  
#### Fix obvious bad data values for daily min-max records (mentioned above).  
#### Add option to turn off all-time record daily min-max lines.  
#### Add animation option to cycle through range of moving average parameters.  
#### When actual temperatures are displayed, the missing values are auto-filled by linear  interpolation, but no extrapolation is done at the edges. Fix this by auto-adjusting the begin and/or end dates so there are no missing values at the edges. This will also allow the high pass filter to work correctly (except those times it doesn't).  
#### Look for a solution for those times the high pass filter fails.  
#### Add capability for user to click on an ad-hoc location on the map view, and have that be the basis for finding temperatures, rather than one of the defined cities.   
#### Add capability to see which year(s) the all-time records occurred, by dwelling cursor on data point.  
#### Display the range of years data has been collected at the weather stations used for the particular display.  
